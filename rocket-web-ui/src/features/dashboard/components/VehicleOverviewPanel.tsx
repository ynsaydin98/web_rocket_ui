    ...
    >
      <div className="vehicle-stage">
        ...
        <div className="vehicle-callout vehicle-callout--alt">
          <span>İRTİFA</span>
          <strong>{telemetry?.irtifa ?? "--"} m</strong>
        </div>
        <div className="vehicle-callout vehicle-callout--speed">
          <span>HIZ</span>
          <strong>{telemetry?.hiz ?? "--"} m/s</strong>
        </div>
        <Suspense fallback={<div className="vehicle-stage__loading">3D sahne hazırlanıyor</div>}>
          <InteractiveRocketScene />
        </Suspense>
        <span className="vehicle-stage__hint">Sürükle: döndür · Tekerlek: yakınlaştır</span>
      </div>
    </Panel>
  );
}
